...
const Racipe = ({ racipes, handleAddToCooks }) => {
    ...
    return (
        <div className='mt-4 lg:p-5'>
            <div className='text-start rounded-3xl border-2 border-[#0BE58A] p-4' >
                <img src={racipes.recipe_img} alt="" />
                <h1 className='text-xl my-2 lg:text-2xl font-bold'>{racipes.name}</h1>
                <p className='my-2 lg:my-6'>{racipes.description}</p>
                <div className='p-2 lg:p-5'>
                    <hr className=' border-cyan-900 text-start justify-start ' />
                </div>
                <h1 className='text-xl my-2'>Ingredients: {racipes.item.length}</h1>
                {
